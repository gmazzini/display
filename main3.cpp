...
// Recovery WiFi/TCP watchdog Rel 20260619
...
static inline void drainClient(void) {
...
static int netFailCount = 0;

static unsigned long netT0 = 0;
static unsigned long backoffMs = 500;
static unsigned long lastFrameMs = 0;
static unsigned long lastWifiRecover = 0;

const int NET_FAILS_BEFORE_WIFI_RECOVER = 5;
const unsigned long WIFI_RECOVER_MIN_GAP = 15000UL;
const unsigned long FRAME_SOFT_TIMEOUT = 30000UL;
const unsigned long FRAME_HARD_TIMEOUT = 180000UL;
const unsigned long WIFI_BOOT_TIMEOUT = 60000UL;
...
static void netTcpResetOnly(void) {
    drainClient();
    client.flush();
    client.stop();
...
static void wifiRecover(void) {
    unsigned long now;

    now = millis();

    if ((unsigned long)(now - lastWifiRecover) < WIFI_RECOVER_MIN_GAP) return;

    lastWifiRecover = now;

    netTcpResetOnly();

    dispIP = IPAddress(0, 0, 0, 0);
    tmp = IPAddress(0, 0, 0, 0);
    lastDns = 0;
...
static void netFailHard(void) {
    ...
    if (netFailCount >= NET_FAILS_BEFORE_WIFI_RECOVER) {
        wifiRecover();
    }
...
void netTask(void *pv) {
    ...
    for (;;) {
        ...
        if ((unsigned long)(now - lastWd) > 500UL) {
            lastWd = now;

            if (valid && lastFrameMs && ((unsigned long)(now - lastFrameMs) > FRAME_SOFT_TIMEOUT)) {
                forceNetRestart = 1;
                lastFrameMs = now;
            }

            if (lastNetOk && ((unsigned long)(now - lastNetOk) > FRAME_HARD_TIMEOUT)) {
                ESP.restart();
            }
...
void setup() {
    ...
    for (;;) {
        ...
        if ((unsigned long)(millis() - lastNetOk) > WIFI_BOOT_TIMEOUT) {
            ESP.restart();
        }
...
void loop() {
    uint32_t mask;
    volatile unsigned long (*frontLocal)[384];

    frontLocal = getFrontPtr();
